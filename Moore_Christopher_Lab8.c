/*
	...
	Date: 3-3-2020
	Lab: #8
	Description: Phonebook; add File funtionality
	user can save contacts to specified file 
	contacts are pulled from contacts file
*/
...
int main()
{
	...
	do //Loop until a user specified file is opened
	{
		printf("Enter Contacts File Name: ");
		gets(file_name);
		
		if(file_name == "\n") //user does not specify a file name
		{
			rContacts = fopen("Contacts.dat","r");
			wContacts = fopen("Contacts.dat","w");
		}
		else 
		{
			rContacts = fopen(file_name,"r");
			wContacts = fopen(file_name,"w");
		}
		
		if(rContacts == NULL || wContacts == NULL)
		{
			printf("File could not be opened.\n");
		}
		
	}while(rContacts == NULL && wContacts == NULL);
	...
	fscanf(rContacts, "%s%s%s",read_first,read_last,read_phone);
	printf("%s %s %s",read_first, read_last, read_phone);
	
	while( !feof(rContacts) )
	{
		strcpy(phoneBook[numStruct].first,read_first);
		strcpy(phoneBook[numStruct].last,read_last);
		strcpy(phoneBook[numStruct].phone,read_phone);
	
		numStruct++;
		if(numStruct == 10)//reallcoates mem for phonebook if needed
		{
			expansion++;
	...
	do //The menu of the phone book
	{
		...
		switch(choice)
		{
			...
			case 0: 
				...
				int k;
				for(k = 0; k < numStruct; k++)
				{
					fprintf(wContacts, "%s %s %s\n",phoneBook[k].first,phoneBook[k].last,phoneBook[k].phone);	
				}
